...
typedef struct { float p, i, d, c, f; } raw_pidcf_t;
  ...
  typedef
      ...
      PIDCF_t<0, PID_MAX, LPQ_MAX_LEN, int(PID_FAN_SCALING_MIN_SPEED), int(PID_FAN_SCALING_LIN_FACTOR)>
    #elif ENABLED(PID_EXTRUSION_SCALING)
      PIDC_t<0, PID_MAX, LPQ_MAX_LEN>
    #elif ENABLED(PID_FAN_SCALING)
      PIDF_t<0, PID_MAX, int(PID_FAN_SCALING_MIN_SPEED), int(PID_FAN_SCALING_LIN_FACTOR)>
    #else
      PID_t<0, PID_MAX>
    #endif
  hotend_pid_t;
